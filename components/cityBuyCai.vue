...
<template>
	<view>
		<!-- 同城买菜 全部、待付款、 -->
		<view class="second">
			<view :class="current==index?'cur1':''" v-for='(value,index) in tcCat' :key='index' @tap="tapcat(index)">
				{{value.name}}
			</view>
		</view>
		<!-- 订单列表 -->
		<view class="list" v-for="(value,index) in infoArr" :key='index'>
			<view class="row1">
				...
				<text class="right" @tap="wancheng" v-if="current==2" >已完成</text>
			</view>
			<view class="row2">
			...
			<view class="row4" @tap="pay" v-if="current==0||current==1">付款</view>
			<view class="row4" @tap="plbtn" v-if="current==2">待评论</view>
			<view class="row4" @tap="tuikuan" v-if="current==3">申请退款</view>
		</view>
	</view>
</template>
